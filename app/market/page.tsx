...
function buildDetailedAnalysis(i: MarketItem) {
  ...
  // ✅ Bardziej “dokładne” sekcje oparte o to co już mamy
  ...
  // Pros/cons w “dokładniejszej” formie: wnioski
  ...
  if (typeof i.views === "number") {
    ...
    else if (i.views >= 1500) conclusions.push("Dużo wyświetleń: warto sprawdzić, czy nie ma „ukrytego haczyka”.");
  ...
  // pytania do sprzedającego (dokładne)
  ...
  const forWho: string[] = [];
  if (ppm2 && hasPrice) {
    forWho.push("Dla kupującego, który chce szybko porównać oferty ceną/m² i negocjować na liczbach.");
  }
          ...
          return (
            <div key={i.id} style={S.card}>
              ...
              <div className="mt-4 grid grid-cols-2 gap-2 text-sm" style={S.soft}>
                ...
                <div>📊 {analysis.ppm2 ? `${fmtMoney(analysis.ppm2)}/m²` : "—"}</div>
                <div>
                  📍 {i.city || "—"}
                  {i.district ? `, ${i.district}` : ""}
                </div>
                <div>👀 Wyświetlenia: {typeof i.views === "number" ? fmtNum(i.views) : "—"}</div>
                <div>{score !== null ? `🏷️ ${scoreLabel(score)}` : "🏷️ Brak oceny"}</div>
              </div>
              ...
              {isOpen && (
                <div className="mt-5 text-sm" style={{ color: "#0f172a" }}>
                  ...
                  <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                    ...
                    <div style={S.boxWarn}>
                      <div className="font-extrabold">⚠️ Minusy / ryzyka</div>
                  ...
                  {/* Pytania */}
                  <div style={S.label}>Pytania do sprzedającego (checklista)</div>
                  <ul className="mt-2 space-y-1" style={S.muted}>
