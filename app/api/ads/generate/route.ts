...
type Body = {
  ...
  city?: string;
  headline?: string;
  offerType?: "sprzedaż" | "kupno" | "wynajem";
  phone?: string;
  brandName?: string;
...
// Bardzo €śreklamowe€ť prompty (czyste, premium, z tekstem czytelnym)
function buildPrompt(style: string, payload: Required<Pick<Body, "city" | "headline" | "offerType" | "phone" | "brandName">>) {
  const offerLabel =
    payload.offerType === "sprzedaż" ? "Sprzedaż nieruchomości" :
    payload.offerType === "kupno" ? "Kupno nieruchomości" :
...
export async function POST(req: Request) {
  try {
    ...
    const n = Math.min(Math.max(pick(raw.n, 1), 1), 2); // max 2 per styl, żeby nie mielić‡ wieczność

    const payload = { city, headline, offerType, phone, brandName } as const;

    // Jedziemy style po stylu, żeby było przewidywalnie i stabilnie
    const results: Array<{
      style: string;
